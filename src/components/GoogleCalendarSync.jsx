...
export function GoogleCalendarSync({ session, onClose }) {
  ...
  const createGoogleCalendarEvent = () => {
    ...
    const eventDescription = `Clock In: ${format(startDate, 'HH:mm:ss')}\\nClock Out: ${format(endDate, 'HH:mm:ss')}\\n\\nTotal Hours: ${session.totalHours.toFixed(2)}h\\nRegular: ${session.regularHours.toFixed(2)}h${session.unpaidExtraHours > 0 ? `\\nUnpaid Extra: ${session.unpaidExtraHours.toFixed(2)}h` : ''}${session.paidExtraHours > 0 ? `\\nPaid Extra: ${session.paidExtraHours.toFixed(2)}h` : ''}`;
          ...
          {!success ? (
            <>
              ...
              <div className="session-preview">
                ...
                <div className="preview-item">
                  <strong>Total:</strong> {session.totalHours.toFixed(2)}h
                </div>
              </div>
            </>
          ) : (
